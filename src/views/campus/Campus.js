...
function Campus() {
    ...
    const getCampusList = () => {
      axios({
        method: 'GET',
        url: `${base_url}/campus/?skip=${skip}&limit=${limit}`,
        headers: {
    ...
    const getUserList = () => {
      axios({
        method: 'GET',
        url: `${base_url}/campus/campususers`,
        headers: {
          'Authorization': 'Bearer ' + token
        }
      }).then((response) => {
        ...
        <div>
          ...
          <div className="modal fade" id="addRegionModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div className="modal-dialog modal-lg">
              <div className="modal-content">
                <div className="modal-header">
                  <h1 className="modal-title fs-5" id="staticBackdropLabel">{mode === 'edit' ? 'Edit campus' : 'Add campus'}</h1>
